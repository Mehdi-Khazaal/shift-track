...
// The UPDATE uses WHERE status='open' to be atomic - only one concurrent call succeeds.
async function processIfExpired(shift) {
  ...
  if (shift.target_type === 'house') {
    ...
    if (!claims.rows.length) {
      await db.query(`UPDATE open_shifts SET status='expired' WHERE id=$1 AND status='open'`, [shift.id]);
      return { ...shift, status: 'expired' };
    }

    const winner = claims.rows[0];

    // Atomic: only the first caller to reach this wins the race
    const claimed = await db.query(
      `UPDATE open_shifts SET status='claimed', claimed_by=$1 WHERE id=$2 AND status='open' RETURNING id`,
      [winner.user_id, shift.id]
    );
    if (!claimed.rows.length) return { ...shift, status: 'claimed' }; // another request got there first
    ...
    await notifyUsers(
      [winner.user_id],
      'Shift Assigned',
      `You got the open shift at ${locName} on ${shift.date} (${shift.start_time.slice(0,5)}-${shift.end_time.slice(0,5)})`
    );
...
router.post('/admin', auth, adminOnly, async (req, res) => {
  ...
  try {
    ...
    console.error('[open-shifts admin POST]', err);
...
router.get('/admin', auth, adminOnly, async (req, res) => {
  try {
    ...
    console.error('[open-shifts admin GET]', err);
    res.status(500).json({ ok: false, error: 'Server error' });
  }
});
...
router.delete('/admin/:id', auth, adminOnly, async (req, res) => {
  try {
    ...
    console.error('[open-shifts admin DELETE]', err);
    res.status(500).json({ ok: false, error: 'Server error' });
  }
});
...
router.get('/', auth, async (req, res) => {
  try {
    ...
    console.error('[open-shifts GET]', err);
    res.status(500).json({ ok: false, error: 'Server error' });
  }
});
...
router.post('/:id/respond', auth, async (req, res) => {
  ...
  try {
    ...
    if (response === 'claimed' && shift.target_type !== 'house') {
      // Atomic claim: WHERE status='open' ensures only one user wins
    ...
    console.error('[open-shifts respond POST]', err);
    res.status(500).json({ ok: false, error: 'Server error' });
  }
});
